...
class TacaUAGameEvents(Base):
    __tablename__ = "tacaua_game"

    id = Column(Integer, primary_key=True, autoincrement=True)
    game_id = Column(Integer, ForeignKey(settings.SCHEMA_NAME + ".tacaua_team.id", name="fk_team1_id"), index=True)
    event = Column(Integer, ForeignKey(settings.SCHEMA_NAME + ".tacaua_team.id", name="fk_team2_id"), index=True)
