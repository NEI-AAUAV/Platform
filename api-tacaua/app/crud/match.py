...
class CRUDMatch(CRUDBase[Match, MatchCreate, MatchUpdate]):
    ...
    def get_last_played(self, db: Session, amount: int):
        matches = db.query(self.model).filter(self.model.date.isnot(None)).order_by(self.model.date).all()

        # Reversing list so that last played game are first
        matches = matches[::-1]

        # Finding first game that was played i.e winner != None
