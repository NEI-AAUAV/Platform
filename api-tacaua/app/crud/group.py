...
class CRUDGroup(CRUDBase[Group, GroupCreate, GroupUpdate]):
    ...
    def update_number(self, db: Session, *, db_obj: Group, number: int) -> Group:
        """Swap the order of two groups."""
        db_obj2 = (
            db.query(Group)
            .filter(
                Group.competition_id == db_obj.competition_id, Group.number == number
            )
            .one_or_none()
        )
        if not db_obj2:
    ...
    def update_teams(self, db: Session, *, db_obj: Group, teams_id: Set[int]) -> Group:
        ...
        """
        modality_id = crud.competition.get(db, id=db_obj.competition_id).modality_id
        ...
        for tid in teams_id_diff:
            team = (
                db.query(Team)
                .filter(Team.id == tid, Team.modality_id == modality_id)
                .first()
            )
            if not team:
        ...
        metadata = crud.competition.get(db, id=db_obj.competition_id)._metadata
        metadata = Metadata.parse_obj(metadata).__root__
        if isinstance(metadata, SingleElimination):
            f = self.update_single_elimination_matches
        elif isinstance(metadata, RoundRobin):
            f = self.update_round_robin_matches
        elif isinstance(metadata, Swiss):
            f = self.update_swiss_matches
        else:
    ...
    def update_single_elimination_matches(
        ...
        try:
            ...
            teams_id_assigned = {
                tid for m in matches for tid in (m.team1_id, m.team2_id) if tid
            }
            teams_id_diff = teams_id - teams_id_assigned

            matches_count_per_round = self.get_single_elimination_matches_per_round(
                len(group.teams)
            )
    ...
    def update_round_robin_matches(
        self, db: Session, group: Group, metadata: RoundRobin
    ) -> None:
        logger.debug(metadata.system)
        ...

    def update_swiss_matches(self, db: Session, group: Group, metadata: Swiss) -> None:
        logger.debug(metadata.system)
        ...

    def get_single_elimination_matches_per_round(self, n_teams: int) -> List[int]:
