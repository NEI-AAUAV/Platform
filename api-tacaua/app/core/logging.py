"""
Original: https://gist.github.com/nkhitrov/a3e31cfcc1b19cba8e1b626276148c49
...
MESSAGE_FORMAT = os.environ.get(
    "MESSAGE_FORMAT",
    "<green>{time:YYYY-MM-DD HH:mm:ss.SSS}</green> | "
    "<level>{level: <8}</level> | "
    "Request ID: <blue>{extra[request_id]}</blue> | "
...
def init_logging():
    ...
    """
    # Redirect all `logging` messages to `loguru`
    ...
    logger.configure(extra={"request_id": "None"})  # Default values
    logger.remove(0)  # Remove default stderr sink
    logger.add(
        sys.stderr,
        filter={"": level_log, "sqlalchemy": logging.WARNING},
        level=0,
