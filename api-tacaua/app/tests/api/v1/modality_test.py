...
def test_create_modality(client: TestClient) -> None:
    r = client.post(URL_PREFIX, json=modality_data, follow_redirects=True)
...
def test_update_modality(db: SessionTesting, client: TestClient) -> None:
    ...
    r = client.put(
        f"{URL_PREFIX}/{modality.id}", json=modality_partial_data, follow_redirects=True
    )
    assert r.status_code == 200
    data = r.json()
    assert "id" in data
...
def test_remove_modality(db: SessionTesting, client: TestClient) -> None:
    ...
    assert len(data["competitions"]) == 1
    competitions = (
        db.query(Competition).filter(Competition.modality_id == modality.id).all()
    )
