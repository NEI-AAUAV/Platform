...
def test_create_group(db: SessionTesting, client: TestClient) -> None:
    ...
    r = client.post(URL_PREFIX, json={"competition_id": id}, follow_redirects=True)
...
@pytest.mark.parametrize("client", [{"scope": "manager-tacaua"}], indirect=True)
def test_update_group_with_team_in_modality(
    db: SessionTesting, client: TestClient
) -> None:
...
@pytest.mark.parametrize("client", [{"scope": "manager-tacaua"}], indirect=True)
def test_update_group_with_team_not_in_modality(
    db: SessionTesting, client: TestClient
) -> None:
    group = db.query(Group).first()
    competition = db.get(Competition, group.competition_id)
...
def test_remove_group(db: SessionTesting, client: TestClient) -> None:
    ...
    competition = (
        db.query(Competition).filter(Competition.id == data["competition_id"]).first()
    )
