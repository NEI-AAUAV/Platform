...
async def create_team(
    ...
    team = await crud.team.update_image(db=db, db_obj=team, image=image)
    return team


@router.put("/{id}", status_code=200, response_model=Team, responses=responses)
...
@router.delete("/{id}", status_code=200, response_model=Team, responses=responses)
def remove_team(
    id: int,
    db: Session = Depends(deps.get_db),
    _=Security(auth.verify_scopes, scopes=[auth.ScopeEnum.MANAGER_TACAUA]),
