...
@router.delete("/{id}", status_code=200, response_model=Group, responses=responses)
def remove_group(
    id: int,
    db: Session = Depends(deps.get_db),
    _=Security(auth.verify_scopes, scopes=[auth.ScopeEnum.MANAGER_TACAUA]),
...
def add_team_to_group(
    ...
    if team.modality_id != competition.modality_id:
        raise HTTPException(
            status_code=400, detail="Team does not belong to the modality of the group."
        )

    group = crud.group.update_teams(
        db=db, db_obj=group, teams_id={standing_group_team.team_id}
    )
