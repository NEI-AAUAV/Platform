...
from fastapi import APIRouter, Depends, Security, UploadFile, File, Request, Form
...
@router.post("/", status_code=201, response_model=Course, responses=responses)
async def create_course(
    ...
    course = await crud.course.update_image(db=db, db_obj=course, image=image)
    return course


@router.get("/{id}", status_code=200, response_model=Course, responses=responses)
def get_course(
    id: int,
    db: Session = Depends(deps.get_db),
    _=Depends(deps.short_cache),
...
@router.put("/{id}", status_code=200, response_model=Course, responses=responses)
...
@router.delete("/{id}", status_code=200, response_model=Course, responses=responses)
def remove_course(
    id: int,
    db: Session = Depends(deps.get_db),
    _=Security(auth.verify_scopes, scopes=[auth.ScopeEnum.MANAGER_TACAUA]),
