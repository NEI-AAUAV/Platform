...
def connection():
    ...
    connection.execute(f'ATTACH DATABASE \'{SQLITE_DB}\' AS {settings.SCHEMA_NAME};')
    Base.metadata.create_all(bind=engine, checkfirst=True) # create the tables
    yield connection
...
def db(connection: Connection) -> Generator[SessionTesting, Any, None]:
    ...
    It is common to also recreate a new database for every test, 
    but only a rollback is faster and sufficient.
    """
